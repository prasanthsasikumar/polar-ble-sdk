...
public class LSL {
    ...
    static {
       // System.setProperty("jna.debug_load", "true");
        ...
        //System.out.println("PLATFORM Mac?: HERE>>>>> " + Platform.isMac());

        if(Platform.isMac())
            inst = (dll) Native.loadLibrary("liblsl64.dylib", dll.class);

        if(Platform.isLinux())
            inst = (dll) Native.loadLibrary("liblsl64.so", dll.class);
        else {
            // For JNA <= 5.1.0
